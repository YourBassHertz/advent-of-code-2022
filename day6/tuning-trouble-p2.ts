...
const findStartingCharForMarker = (datastream: string): number => {
  let startingCharPosition = 0

  const startingPoint = 0
  const endingPoint = 14
  ...
  for (let index = 0; index < datastream.length; index++) {
    ...
    // console.log('testing data stream', testingDataStream)
    // console.log('isUnique', isUnique(testingDataStream))
...
console.log('start-of-message marker location', startingCharPosition)